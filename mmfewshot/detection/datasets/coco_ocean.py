...
COCOO_SPLIT = dict(
    ALL_CLASSES=('fish','jellyfish','penguin','puffin','shark','starfish','stingray'),
    NOVEL_CLASSES=('shark','starfish','stingray'),
    BASE_CLASSES=('fish','jellyfish','penguin','puffin'))


@DATASETS.register_module()
class FewShotCocoODataset(BaseFewShotDataset, CocoDataset):
    ...
    def evaluate(self,
        ...
        Args:
            ...
            class_splits: (list[str] | None): Calculate metric of classes split
                in COCOO_SPLIT. For example: ['BASE_CLASSES', 'NOVEL_CLASSES'].
                Default: None.
...
#         pdb.set_trace()
        ...
        for metric in metrics:
            ...
            if class_splits is not None:
                class_splits = {k: COCOO_SPLIT[k] for k in class_splits}
...
@DATASETS.register_module()
class FewShotCocoOCopyDataset(FewShotCocoODataset):
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]) -> List[Dict]:
        ...
        if isinstance(ann_cfg, dict):
            assert ann_cfg.get('data_infos', None) is not None, \
                'ann_cfg of FewShotCocoOCopyDataset require data_infos.'
...
@DATASETS.register_module()
class FewShotCocoODefaultDataset(FewShotCocoODataset):
    ...
    coco_benchmark = {
        f'{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'splits/seed1/'
                f'full_box_{shot}shot_{class_name}.json')
            for class_name in COCOO_SPLIT['ALL_CLASSES']
        ]
        for shot in [10, 30]
    }
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(FewShotCocoODataset, self).ann_cfg_parser(new_ann_cfg)
